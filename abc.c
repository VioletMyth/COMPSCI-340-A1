...
int main(int argc, char *argv[]) {
    ...
    start_block.data = (int *)mmap(NULL, sizeof(int)*size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS , -1, 0);

    if (start_block.data == (int *)-1) {
        perror("can't allocate shared data");
        exit(EXIT_FAILURE);
    }
